...
const images = document.getElementsByTagName("img")[2];
console.log(images.style.width);
console.log(images.style.height);
const link1 = document.getElementsByClassName("nav-scroll");
const link2 = document.getElementsByClassName("navbar-scroll");

const addEvent = (links) => {
  for (let link of links) {
    link.addEventListener("click", (element, elemen1) => {
      window.scrollTo(1000, 1000);
      console.log(element, elemen1);
    });
  }
};
addEvent(link1);
...
form.addEventListener("submit", async (event) => {
  ...
  const details = { email: event.srcElement["input"].value };

  if (details.email) {
    try {
      const res = await axios.post(
        "https://ecommerce-vegies-api.herokuapp.com/",
        details
      );
      const alert = document.getElementById("alert") || location.reload();
      alert.classList.remove("d-none");
      alert.classList.remove("alert-danger");
      alert.classList.add("alert-success");

      alert.children[0].innerText = `${details.email}`;
      alert.childNodes[2].data = " email added successfully";
      window.scrollTo(0, 0);
    } catch (error) {
...
if (screen.width <= 768) {
  ...
  footer.classList.remove("rounded-pill");
